...
void TxnStats::add_stats(txnid_t txn_id,TXN_STATS_TYPE type,void * value)
{
    ...
    switch (type)
    {
        ...
        case RESULT:
            ...
            break;
        case GET_QUERY_TIME:
            txn_stats->get_query_time = *(uint64_t*)value;
            break;
        case INDEX_TIME:
            txn_stats->index_time = *(uint64_t*)value;
            break;
        case CC_TIME:
            txn_stats->cc_time = *(uint64_t*)value;
            break;
        default:
...
void TxnStats::txn_finish(txn_man * txn,base_query* query,RC rc,uint64_t timespan,uint64_t start_time,uint64_t query_time,RunInfor infor)
{
   ...
   /* if(txn_infor_map.find(txn_id) == txn_infor_map.end())
    ...
    add_stats(txn_id,GET_QUERY_TIME,&query_time);
    ...
    add_stats(txn_id,INDEX_TIME,&infor.index_time);
    add_stats(txn_id,CC_TIME,&infor.cc_time);
#endif
...
void TxnStats::stats_print()
{
    for(TxnMap::iterator i = txn_infor_map.begin(); i != txn_infor_map.end(); i++)
    {
        ...
        cout << "GET_QUERY_TIME = " << now_stats->get_query_time << endl;
        cout << "INDEX_TIME = " << now_stats->index_time << endl;
        cout << "CC_TIME = " << now_stats->cc_time << endl;
    }
